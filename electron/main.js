...
// ─── Database Init ──────────────────────────────────────────────────────────
// Each license key gets its own isolated SQLite database file.
// DB filename = paxxmo-{first 8 chars of SHA256(normalizedKey)}.db
// Before any license is activated, we use paxxmo-default.db (anonymous mode).

function getLicenseDbPath(licenseKey) {
  const clean = String(licenseKey || '').trim().toUpperCase();
  if (!clean) return path.join(app.getPath('userData'), 'paxxmo-default.db');
  const hash = crypto.createHash('sha256').update(clean).digest('hex').substring(0, 12);
  return path.join(app.getPath('userData'), `paxxmo-${hash}.db`);
}
...
function seedDefaultUsers(dbInstance) {
  try {
    const insert = dbInstance.prepare(`
      INSERT OR IGNORE INTO system_users (id, username, password_hash, barcode, role, name)
    ...
    dbInstance.prepare("UPDATE system_users SET barcode = 'USER-super_admin-u1' WHERE username = 'superadmin' AND (barcode IS NULL OR barcode = 'DEV-0000')").run();
    dbInstance.prepare("UPDATE system_users SET barcode = 'USER-owner-u2'       WHERE username = 'admin'      AND (barcode IS NULL OR barcode = 'OWN-1234')").run();
    dbInstance.prepare("UPDATE system_users SET barcode = 'USER-manager-u3'     WHERE username = 'manager'    AND (barcode IS NULL OR barcode = 'MGR-5678')").run();
    dbInstance.prepare("UPDATE system_users SET barcode = 'USER-staff-u4'       WHERE username = 'cashier'    AND (barcode IS NULL OR barcode = 'STF-1111')").run();
  } catch(e) {
    console.warn('[DB Seeding] Failed to ensure default users:', e);
  }
}

// ─── Initialize a DB instance: apply schema + seed ──────────────────────────
...
// ─── Active DB reference (switched per license) ──────────────────────────────
let db = initializeDatabase(new Database(getLicenseDbPath('')));
let currentDbPath = getLicenseDbPath('');

// ─── Switch database to the one for a given license key ─────────────────────
function switchDatabase(licenseKey) {
  const targetPath = getLicenseDbPath(licenseKey);
  if (targetPath === currentDbPath) {
    console.log('[DB] Already on correct database:', path.basename(targetPath));
    return path.basename(targetPath);
  }
  try {
    // Close current DB cleanly
    try { db.close(); } catch (_) {}
    // Open or create the license-specific database
    const newDb = new Database(targetPath);
    initializeDatabase(newDb);
    db = newDb;
    currentDbPath = targetPath;
    console.log('[DB] Switched to:', path.basename(targetPath));
  } catch (err) {
    console.error('[DB] switchDatabase failed:', err.message);
    // Fallback: keep using old db if switch fails
  }
  return path.basename(currentDbPath);
}

// ─── IPC: switch-database ─────────────────────────────────────────────────────
// Called by the renderer whenever a license key is activated or app starts.
ipcMain.handle('switch-database', (event, { licenseKey } = {}) => {
  const dbFile = switchDatabase(licenseKey || '');
  return { success: true, dbFile };
});

// ─── IPC: get-current-db-info ─────────────────────────────────────────────────
ipcMain.handle('get-current-db-info', () => {
  return { dbFile: path.basename(currentDbPath), dbPath: currentDbPath };
});

// ─── IPC: reset-business-data ────────────────────────────────────────────────
// Wipes all business data from the CURRENT (license-specific) database.
// System users are preserved so staff can still log in.
ipcMain.handle('reset-business-data', () => {
  try {
    db.exec(`
      ...
      DELETE FROM kots;
      DELETE FROM recipes;
      DELETE FROM el_products;
      DELETE FROM el_serials;
      DELETE FROM el_suppliers;
      DELETE FROM el_grns;
      DELETE FROM el_sales;
      DELETE FROM repair_jobs;
      DELETE FROM el_customers;
    ...
    console.error('[reset-business-data]', err);
...
// ─── Global Barcode Lookup (no CORS in main process) ────────────────────────
// Multi-source fallback: Open Food Facts → UPCitemdb → Open PLM
ipcMain.handle('barcode-lookup', async (event, { barcode }) => {
  ...
  // Helper: tidy up a product name string
  ...
  function titleCase(str) {
    return String(str || '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()).trim();
  }

  try {
    // ── 1. Open Food Facts (best coverage for grocery/food) ──────────────────
    try {
      ...
      if (res.ok) {
        ...
        if (data.status === 1 && data.product?.product_name) {
          ...
          const name = cleanName(p.brands, p.product_name, p.quantity);
          const rawCat = (p.categories_tags || []).find(c => !c.includes(':')) ||
            (p.categories_tags || [])[0]?.replace(/^[a-z]{2}:/, '') || '';
          const category = titleCase(rawCat);
          cleanUp();
          return { found: true, name, category, source: 'Open Food Facts' };
        }
      }
    } catch (e) {
      if (e.name === 'AbortError') { cleanUp(); return { found: false }; }
      // Continue to next source
    }

    // ── 2. UPC Item DB (covers many retail/non-food products) ───────────────
    try {
      ...
      if (res2.ok) {
        ...
        if (item && item.title) {
          ...
          return {
            found: true,
            name: cleanName(item.brand, item.title),
    ...
    // ── 3. Open PLM (electronics, SKUs, manufactured goods) ─────────────────
    try {
      const res3 = await fetch(
        `https://www.digit-eyes.com/gtin/aHR0cHM6Ly93d3cuZGlnaXQtZXllcy5jb20vc2VydmljZXMvdjIvbG9va3VwLmpzb24/dXBjPQ==${encodeURIComponent(code)}`,
        { signal: controller.signal }
      );
      // digit-eyes requires key; use as placeholder — skip to barcodelookup.com
    } catch (_) {}

    // ── 4. Barcode Lookup (broad coverage, no key needed) ───────────────────
    try {
      const res4 = await fetch(
        `https://barcodelookup.com/${encodeURIComponent(code)}`,
        {
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; PaxxmoPOS/1.0)',
      ...
      if (res4.ok) {
        ...
        // Scrape product name from meta tag (last resort)
        const match = text.match(/<meta\s+(?:property="og:title"|name="title")\s+content="([^"]+)"/i);
        if (match && match[1] && !match[1].toLowerCase().includes('barcode lookup')) {
          cleanUp();
          return { found: true, name: match[1].trim(), category: '', source: 'Barcode Lookup' };
        }
      }
    } catch (_) {}
  ...
  try {
    ...
    const resultPromise = new Promise((resolve) => {
      ...
      worker.webContents.once('did-finish-load', async () => {
        ...
        let finalPageSize = isA4 ? 'A4' : pageSizeMicrons;

        if (!isA4) {
          try {
            // Measure actual content height
            const documentHeight = await worker.webContents.executeJavaScript(
              'Math.max(document.documentElement.scrollHeight, document.body.scrollHeight, 100)'
            );
            
            // Resize worker window to fit content exactly (plus 30px safety padding to prevent cutoff)
            const paddingPx = 30;
            const finalHeight = documentHeight + paddingPx;
            worker.setSize(windowWidth, finalHeight);

            // Convert to microns (1px = 25400 / 96 microns ≈ 264.5833 microns)
            const heightMicrons = Math.ceil(finalHeight * 264.583);
            ...
            console.log(`[Print] Dynamic receipt height contentHeight=${documentHeight}px  finalHeight=${finalHeight}px  heightMicrons=${heightMicrons}`);
          } catch (heightErr) {
            console.warn('[Print] Failed to calculate dynamic height, using default:', heightErr);
          }
        }
